...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="koj1986/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/koj1986/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@3.36.89.8 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@3.36.89.8 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'koj1986.j@gmail.com')
